...
El primer módulo en que se realiza la descarga del dataset, la división de \textit{train} y \textit{test}, y el preprocesamiento. Es decir, este módulo abarca las tareas realizadas en los primeros tres directorios del ambiente local.

En el segundo módulo es en el que se realiza la optimización de hiperparámetros y el entrenamiento de los modelos de \textit{machine learning}. Si bien estas dos tareas son secuenciales, las ejecuciones correspondientes a cada modelo se realizan en paralelo. Como salida se obtienen los tres modelos, que son utilizados por los módulos de \textit{tracking server} y "mejor modelo".
...
Si bien este método ayuda a reducir la cantidad de datos atípicos, estos no serán siempre cero. Por ejemplo, la variable \texttt{total asset turnover} se trató con la transformación de Yeo-Johnson, la que redujo los \textit{outliers} de 247 a cero, tal como se ve en la figura \ref{fig:transformacion-yeo-johnson}.
...
Se implementa mediante la clase \texttt{LogisticRegression} \citep{WEBSITE:sklearn_LogisticRegression} de \texttt{scikit-learn}. En particular, se optimizan los siguientes hiperparámetros:
...
\begin{itemize}
    ...
    \item \texttt{Penalty} o penalización: determina que tipo de regularización utilizar. \texttt{L1} (Lasso) es excelente para selección de variables, mientras que \texttt{L2} (Ridge) es el estándar para estabilidad. Se exploran ambos valores posibles.
...
\begin{itemize}
    \item Elección de un porcentaje de \textit{features}: se logra mediante la clase \texttt{SelectPercentile} \citep{WEBSITE:sklearn_SelectPercentile}. Para ello se define el parámetro \texttt{percentile}, y se explora en el rango $50$ y $80$, es decir, entre el 50\% y 80\%.
...
El último modelo estudiado es XGBoost \citep{model:xgboost}. Este es un algoritmo de \textit{ensemble learning} basado en árboles de decisión que construye modelos de forma secuencial para corregir los errores de los anteriores. Esto hace que, en problemas con datos estructurados (como los de este trabajo), supere consistentemente a otros algoritmos. Otro de los motivos de su elección es que ayuda a prevenir el \textit{overfitting}, ya que incluye penalizaciones L1 (Lasso) y L2 (Ridge) en su función objetivo. Finalmente, este algoritmo presenta el parámetro \texttt{scale\_pos\_weight}, que permite tratar el desbalance al dar más peso a una clase minoritaria, lo cual resulta ideal para el dataset utilizado.
...
Los mejores hiperparémtros encontrados para XGBoost se listan en la tabla \ref{tab:xgboost-hip}.
...
Luego de optimizar los hiperparámetros y entrenar a cada modelo, se obtienen métricas de \textit{recall}, \textit{precision}, \textit{f1-score}, \textit{f2-score}, \textit{AUC-ROC} y \textit{average precision}. Estas se registran en el \textit{tracking server} para poder ser utilizadas en pasos posteriores.
...
En la configuración actual del trabajo se utiliza la métrica de \textit{average precision}, misma métrica que se usa para optimizar hiperparámetros, la cual pone como mejor modelo a XGBoost.
...
Respecto al \textit{model registry}, este permite registrar aquellos modelos que se utilizan en la versión productiva del trabajo, es decor, los mejores modelos o \textit{champion}. Técnicamente, se logra mediante la clase \texttt{mlflow.client} \citep{WEBSITE:mlflow_client_api}, con los siguientes métodos:
\begin{itemize}
...
El primer DAG definido es \texttt{download\_and\_split\_dataset}, el cual se observa en la figura \ref{fig:dag01}. Su propósito es el de descargar el dataset \textit{Taiwanese Bankruptcy Prediction} y hacer su división en \textit{train} y \textit{test}.
...
Para esto, se divide el DAG en dos tareas, las cuales se ejecutan secuencialmente:

\begin{itemize}
    \item \texttt{download\_and\_load\_dataset}: descarga el dataset original y lo guarda en una base de datos S3 (el cual está dockerizado).
    \item \texttt{split\_raw\_dataset}: Toma el dataset original de S3, lo divide en \textit{train} y \textit{test}. Finalmente, guarda estos dos datasets nuevos en S3, para ser utilizados en otros DAGs.
\end{itemize}

\subsection{DAG - model training}

El segundo DAG es \texttt{model\_training}, el cual se observa en la figura \ref{fig:dag02}. Su propósito es optimizar los hiperparámetros de los modelos, entrenar los modelos y finalmente elegir al mejor de ellos.
...
Este DAG se caracteriza por ejecutar tres tareas en paralelo, las cuales convergen en una tarea final. Cada una de estas leen los dataset de \textit{train} y \textit{test} originados en el DAG anterior.

Las tareas que se ejecutan en paralelo, que a su vez ejecutan internamente dos tareas secuenciales, son:
...
El servicio web se implementa mediante el framework FastAPI \citep{WEBSITE:fastapi}. El mismo consta de los siguientes endpoints:
...
\begin{itemize}
    ...
    \item \texttt{get\_experiment\_image}: método \texttt{get} que permite obtener una imagen asociada a cualquiera de las últimas versions de los modelos entrenados, siendo estos \textit{logistic regression}, SVM y XGBoost. Devuelve el mismo tipo de imágenes que el método \texttt{get\_champion\_image}.
