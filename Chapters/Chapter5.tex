...
En este capítulo se repasan brevemente las tareas realizadas para realizar este trabajo. Finalmente, se detallan una serie de posibles mejora a futuro con el fin de mejorar el desarrollo actual.
...
En cuanto a modelos, se estudiaron \textit{Logistic Regression}, SVM y XGBoost, como así también se optimizaron sus hiperparámetros para obtener una versión óptima. En este caso, fue fundamental el uso de modelos basados en \textit{pipelines} de \textit{imblearn}, ya que mejoraron el rendimiento de todos los modelos. A modo de ejemplo, se menciona el \textit{recall} de \textit{XGBoost}, donde la versión normal obtuvo un 0,4242 y la versión con \textit{pipeline} un 0,7727.

Con las herramientas de \textit{airflow} y MLFlow se obtuvo un ambiente automatizado, reproducible, rastreable y escalable. Esto permitió realizar diversas iteraciones, donde se estudiaron y mejoraron los modelos de \textit{machine learning}. Finalmente, se implementó un servicio web que permite realizar predicciones con el mejor de estos modelos.
...
\begin{itemize}
    \item Refinamiento de los modelos actuales: si bien los modelos utilizados arrojan buenos resultados, una posible mejora consiste en estudiar otras técnicas, ya sean para procesar el dataset o para optimizar hiperparámetros, con el fin de obtener mejores métricas finales. Por ejemplo, se podrían imputar los \textit{outliers} mediante métodos más robustos basados en \textit{Random Forest} o \textit{Gradient Boosting}.
