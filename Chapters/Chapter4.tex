...
En la sección \ref{sec:model_training} se detalla al DAG de \texttt{model training}. Este está compuesto por tres tareas que se ejecutan en paralelo, \texttt{logistic\_regression\_training}, que entrena un modelo \textit{Logistic Regression}, \texttt{svm\_training}, que entrena un modelo SVM, y \texttt{xgboost\_training}, que entrena un modelo XGBoost. Una vez finalizadas estas tareas, se ejecuta una tarea final, \texttt{seleccionar\_champion}, cuyo objetivo es el de seleccionar al mejor modelo.
...
En la figura \ref{fig:validacion_dag02} se observan los resultados del DAG \texttt{model training}, en donde se detalla que cada tarea tuvo un resultado satisfactorio.
...
Se utiliza el dataset de \textit{Polish Companies Bankruptcy} \citep{dataset:polish_companies_bankruptcy_365}, que contiene información financiera de empresas polacas entre los años 2000 y 2012. La particularidad de este dataset es que tiene menos columnas que el dataset de Taiwán, ya que solo 11 son equivalentes. Estas se detallan en la tabla \ref{tab:polish-taiwan}.
...
En cuanto a registros, el dataset de Polonia cuenta con 7027, donde 6756 corresponden a empresas que no entran en quiebra, mientras que los 271 restantes corresponden a empresas que sí entran en quiebra.
...
En resumen, el modelo entrenado con el dataset de Taiwán puede detectar quiebras asociadas al mercado ruso, pero es sensible a clasificar erróneamente a empresas sanas. Esta impresición puede deberse al formato del dataset ruso, que solamente tiene $11$ columnas equivalentes con el dataset de Taiwán. De manera similar al caso Polonia, pueden haber también diferentes legislaciones y/o definiciones que inciden en la definición de una quiebra.