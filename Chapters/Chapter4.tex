...
La matriz indica que el modelo obtiene un \textit{recall} o \textit{true positive rate} de $0,77$, es decir, que el modelo detecta de manera satisfactoria al $77\%$ de las empresas que entran en quiebra. El $23\%$ que resta se clasifica de manera errónea, es decir, el modelo indica que no entra en quiebra. Esto se ve en el valor de \textit{false negative rate}, que es de $0,23$.
...
\begin{figure}[H]
	\centering
	\includegraphics[width=0.95\textwidth]{./Figures/fig_precision_recall_curve.png}
...
En la tabla \ref{tab:var_importance_best_model} se observa que las variables \texttt{Working Capital/Equity} y \texttt{Net Value Per Share (C)} acumulan cerca del $26\%$ de la ganancia, por lo que son las que más le permiten al modelo definir una quiebra. Es decir, el modelo considera una quiebra cuando una empresa agota el capital de trabajo operativo que respaldan sus socios y cuando su valor de liquidación por acción disminuye o incluso se desploma.

Por su parte, las variables \texttt{Inventory Turnover Rate (times)}, \texttt{Cash Flow to Total Assets} y \texttt{Interest-bearing debt interest rate} acumulan cerca del $22\%$ de la ganancia del modelo. El modelo penaliza a las empresas que no rotan sus inventarios y que no generan caja real con sus activos. Esta penalización es aún mayor si las empresas deben pagar tasas altas de interés a sus acreedores.

En general, las variables más importantes del modelo indican que este determina la quiebra a partir de la liquidez inmediata y la eficiencia del ciclo de efectivo de una empresa dada.
